...
public class ViewDragHelper {
    ...
    private int[] mEdgeSize = new int[] {EDGE_SIZE, EDGE_SIZE, EDGE_SIZE, EDGE_SIZE};
    ...
    private ViewDragHelper(Context context, ViewGroup forParent, Callback cb) {
        ...
        for (int i = 0; i < 4; ++i) {
            mEdgeSize[i] = (int) (mEdgeSize[i] * density + 0.5f);
        }
    ...
    public int getEdgeSize() {
        return mEdgeSize[getIndexByEdgeFlags(mTrackingEdges)];
    }
    ...
    public void setEdgeSize(int edgeSize) {
        if (mTrackingEdges == EDGE_ALL || mTrackingEdges == 0) {
            for (int i = 0; i < 4; ++i) {
                mEdgeSize[i] = edgeSize;
            }
        } else {
            mEdgeSize[getIndexByEdgeFlags(mTrackingEdges)] = edgeSize;
        }
    }

    private int getIndexByEdgeFlags(int edgeFlags) {
        switch (edgeFlags) {
            case EDGE_LEFT:
                return 0;
            case EDGE_RIGHT:
    ...
    void setDraggingState(MotionEvent ev, int dragsStarted) {
        final int pointerCount = MotionEventCompat.getPointerCount(ev);
        for (int i = 0; i < pointerCount; i++) {
            final int pointerId = MotionEventCompat.getPointerId(ev, i);
            if (!isValidPointerForActionMove(pointerId)) {
            ...
            if (tryCaptureViewForDrag(toCapture, pointerId, true)) {
                saveLastMotion(ev);
                break;
            }
        }
    ...
    boolean tryCaptureViewForDrag(View toCapture, int pointerId) {
        return tryCaptureViewForDrag(toCapture, pointerId, false);
    }

    boolean tryCaptureViewForDrag(View toCapture, int pointerId, boolean force) {
        if (toCapture == mCapturedView && mActivePointerId == pointerId) {
            // Already done!
            return true;
        }
        if (toCapture != null && (force || mCallback.tryCaptureView(toCapture, pointerId))) {
    ...
    public void processTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                if (mDragState == STATE_DRAGGING && mActivePointerId >= 0) {
                    // If pointer is invalid then skip the ACTION_MOVE.
    ...
    private int getEdgeTouched(int x, int y) {
        ...
        if (x < mParentView.getLeft() + mEdgeSize[0])
            result = EDGE_LEFT;
        if (y < mParentView.getTop() + mEdgeSize[3])
            result = EDGE_TOP;
        if (x > mParentView.getRight() - mEdgeSize[1])
            result = EDGE_RIGHT;
        if (y > mParentView.getBottom() - mEdgeSize[2])
            result = EDGE_BOTTOM;
